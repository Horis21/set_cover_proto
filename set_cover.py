...
if __name__ == "__main__":
    ...
    results = pd.DataFrame(columns=[    #     'name', 'size', 'depth, 'explored', 'elapsed_time', avg_question_length'
                ])
    for name in names:
        filename = name
        name = name.split("/")[-1]
        ...
        solver = Solver(name = name.split("/")[-1].split("\\")[-1], sample_size= int(0.66 * len(df)), MIP_gap= 0.3, cover_runs=7, lower_bound_strategy="set-cover")
