...
int main() {
    ...
    int total_instances = 810;  // Number of instances in the dataset
    int features = 18;          // Number of features (excluding the label column)

    // Array to hold the dataset (including labels)
    float dataset[total_instances][features + 1];  // +1 for the label column

    // Open the dataset file
    FILE *file = fopen("10rooms_8_versus_all.csv", "r");
    if (file == NULL) {
