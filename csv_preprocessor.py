...
def convert_csv_from_witty(input_file, output_file):
    # Read the space-separated CSV file
    df = pd.read_csv(input_file, sep=',', header=None)
    ...
    # Swap the first column with the last column
    cols = df.columns.tolist()
    cols.insert(0, cols.pop())  # Move first column to the end
    df = df[cols]
...
for filename in os.scandir(directory):
    if filename.is_file():
        ...
        output_csv =  'sampled_from_witty/' + filename.path.split("/")[-1].split("\\")[-1] +'_bobotree.csv'
        convert_csv_from_witty(input_csv, output_csv)
